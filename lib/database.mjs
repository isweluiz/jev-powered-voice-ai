...
export function createPostgresStore(connectionString, { schema = 'public' } = {}) {
  ...
  return {
    ...
    async saveAgent(userId, agent, { create = false } = {}) {
      const { rows } = create
        ? await pool.query('INSERT INTO agents(id,user_id,configuration) VALUES ($1,$2,$3::jsonb) RETURNING configuration', [agent.id, userId, JSON.stringify(agent)])
