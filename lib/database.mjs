...
export function createPostgresStore(connectionString, { schema = 'public' } = {}) {
  ...
  pool.on('error', () => console.error('Osprey database connection interrupted.'));
  return {
    async init() {
      ...
      try {
        ...
        const applied = await client.query('SELECT id FROM schema_migrations WHERE id = $1', ['001-auth']);
        if (!applied.rowCount) {
          await client.query(await readFile(new URL('../migrations/001-auth.sql', import.meta.url), 'utf8'));
          await client.query('INSERT INTO schema_migrations(id) VALUES ($1)', ['001-auth']);
        }
