...
import { TEMPLATES, getTemplate, buildPrompt } from '../public/templates.js';
...
export async function listWorkspaceAgents(store, owner, defaults) {
  const existing = await store.listAgents(owner);
  const ids = new Set(existing.map(agent => agent.id));
  for (const template of TEMPLATES) {
    // Stable, owner-specific UUIDs make provisioning safe across tabs and servers.
    // Keep this namespace stable so renamed or customized starters are never reset.
    const bytes = createHash('sha256').update(JSON.stringify(['cayana-starter-v1', owner, template.id])).digest().subarray(0, 16);
    bytes[6] = (bytes[6] & 0x0f) | 0x80;
    bytes[8] = (bytes[8] & 0x3f) | 0x80;
    const id = bytes.toString('hex').replace(/^(.{8})(.{4})(.{4})(.{4})(.{12})$/, '$1-$2-$3-$4-$5');
    if (ids.has(id)) continue;
    const agent = { ...newAgent(validateAgent({ name: template.name, templateId: template.id }, defaults)), id };
    await store.saveAgent(owner, agent, { create: true, ifAbsent: true });
  }
  return store.listAgents(owner);
}
...
export function createLocalAgentStore(directory) {
  ...
  return {
    ...
    saveAgent(owner, agent, { create = false, ifAbsent = false } = {}) {
      const result = queue.then(async () => {
        const data = await read();
        const index = data.findIndex(row => row.owner === owner && row.agent.id === agent.id);
        if (index >= 0 && create && ifAbsent) return data[index].agent;
