...
export function mountShell({ active = 'home', onSettings = () => {} } = {}) {
  ...
  const nav = [['home', '/', 'Home'], ['mic', '/talk', 'Talk'], ['agents', '/agents', 'My agents'], ['grid', '/templates', 'Templates']];
  const keys = ['home', 'talk', 'agents', 'templates'];
  ...
  aside.innerHTML = `<a class="shell-brand" href="/" aria-label="Cayana home"><span>${icon('brand')}</span>Cayana</a>
    ...
    <button type="button" data-theme-toggle>${icon('sun')}<span>${document.documentElement.dataset.theme === 'dark' ? 'Light theme' : 'Dark theme'}</span></button>
